defmodule Day16 do
  ...
  def decode_packet(<<version::3, @literal_type::3, rest::bits>>, count, version_sum) do
    decode_literal(rest, <<>>, count + 6, version_sum + version)
  end

  def decode_packet(<<version::3, operator_type::3, rest::bits>>, count, version_sum) do
  ...
  def decode_literal(<<@literal_part::1, group::bits-4, rest::bits>>, acc, count, version_sum) do
    decode_literal(rest, <<acc::bits, group::bits>>, count + 5, version_sum)
  end

  def decode_literal(<<@literal_end::1, group::bits-4, rest::bits>>, acc, count, version_sum) do
  ...
  def decode_sub_packets_by(_method, 0, decoded_packets, bits, count, version_sum) do
    {Enum.reverse(decoded_packets), bits, count, version_sum}
  end

  def decode_sub_packets_by(method, remaining, decoded_packets, bits, count, version_sum) do
